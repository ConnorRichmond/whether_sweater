...
RSpec.describe BookSearchSerializer, :vcr do
  it "formats results for the book search endpoint" do
    forecast = WeatherFacade.new.get_forecast("vermont")
    book_results = BookSearchFacade.new.find_books({location: "vermont", quantity: 5})
    ...
    expect(serialized[:data][:attributes][:destination]).to eq("vermont")
